...
if __name__ == '__main__':
    ...
    parser.add_argument('inputFile'        , nargs='?', help = 'Single json file or yaml config containing list of json files')
    parser.add_argument('--executor','-x'  , default='ddr', help = 'Which executor to use')
    parser.add_argument('--prefix', default='', help = 'Prefix or redirector to look for the files')
    ...
    if executor == 'ddr': 
        ...
        ddr = CoffeaDynamicDataReduction(
            ...
            #             {'object_path': "Events",
            #              'num_entries': 1000},}}},
            data = dataset_tools.preprocess(data_dict, scheduler=mgr)[0], #only grab out_available not out_updated 
            processors = {
        ...
        outpath = "."
        if not os.path.isdir(outpath): os.system("mkdir -p %s"%outpath) #TODO: use something simiilar for results dir
