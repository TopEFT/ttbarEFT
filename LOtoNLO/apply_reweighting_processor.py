...
from ttbarEFT.modules.processor_tools import calc_eft_weights, get_syst_lists, get_sum_mass, get_sum_pt
...
class AnalysisProcessor(processor.ProcessorABC):
    def __init__(self, samples, lep_cat, wc_names_lst=[], hist_lst=None, do_errors=False, doPDF=False, doSR=False, syst_list=[], dtype=np.float32):
        ...
        if self._doSR:
            axes = axes_info['SR_axes']
        else: 
            axes = axes_info['CR_axes']
        ...
        if self._doPDF:
            histograms['LHEPDFweights'] = hist.Hist(
                proc_axis, 
                chan_axis, 
                hist.axis.Variable(axes['mllbb']['variable'], name='mllbb', label='mllbb [GeV]'),
    ...
    def process(self, events):
        ...
        # lldphi = abs(l0.phi - l1.phi) - np.pi
        lldphi = np.arccos(np.cos(abs(l0.phi - l1.phi)))
        ...
        for kinematic_var in kinematic_variations: 
            ...
            # Use these function for manual calculations
            mllbb = get_sum_mass([l0, l1, b0, b1])
            ptllbb = get_sum_pt([l0, l1, b0, b1])
            ...
            dense_axis_variables['ptllbb'] = ptllbb
            dense_axis_variables['lldphi'] = lldphi
            ...
            for wgt_fluct in wgt_var_lst: 
                ...
                for chan_id, chan_settings in channels.items():
                    ...
                    for dense_axis_name, dense_axis_vals in dense_axis_variables.items():
                        ...
                        if ('bmask_exactly2med' not in cuts_list) and (dense_axis_name == 'mllbb' or dense_axis_name=='ptllbb'):
                            continue
