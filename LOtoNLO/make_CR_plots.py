...
def group_hist_processes(h, process_map, others_name="Others"):
    ...
    for new_name, old_names in process_map.items():
        ...
        if existing_olds:
            active_groups[new_name] = existing_olds
    ...
    final_categories = list(active_groups.keys())
    if remaining_procs:
...
def make_MC_comps_withsyst(year, hists_num, hists_denom, var_list=[], syst_list=[], procs_to_group=process_grouping, fig_syst_label='all_syst', ylog=False, do_mcerr=True, plottitle='', figtitle=None, outdir='.'):
    ...
    for var in var_list:
        ...
        h_num_allch = hists_num[var].as_hist({})
        h_denom_allch = hists_denom[var].as_hist({})

        for ch in list(h_num_allch.axes['channel']):
            h_num = h_num_allch[{'channel':ch}]
            h_denom = h_denom_allch[{'channel':ch}]
            ...
            plt.figtext(0.14, 0.84, ch, fontsize=20, fontstyle='italic')
            ax.set_title(plottitle, fontsize=20, pad=40)

            figname = f"{figtitle}{ch}_{var}"
...
def make_3MC_comps_withsyst(year, hists_num, hists_num2, hists_denom, var_list=[], syst_list=[], procs_to_group=process_grouping, fig_syst_label='all_syst', ylog=False, do_mcerr=True, plottitle='', figtitle=None, outdir='.'):
    ...
    for var in var_list:
        ...
        h_num_allch = hists_num[var].as_hist({})
        h_num2_allch = hists_num2[var].as_hist({})
        h_denom_allch = hists_denom[var].as_hist({})

        for ch in list(h_num_allch.axes['channel']):
            h_num = h_num_allch[{'channel':ch}]
            h_num2 = h_num2_allch[{'channel':ch}]
            h_denom = h_denom_allch[{'channel':ch}] 
...
def run_LOtoNLO_mllbb_plots_3MC(args, hist_dict_num, hist_dict_num2, hist_dict_denom, outtitle):
